...
const usuariosForcaPath = `./armor/jogo/forca/usuarios.json`
...
const existIdForcaGameInUsu = (sender, from) => {
     AB = usuarios_forca.map(i => i.id).indexOf(sender)
     data = JSON.parse(fs.readFileSync(`./armor/jogo/forca/${from}.json`))
     nmr = 0
...
const forcaWordPath = `./armor/jogo/forca/palavras.json`
